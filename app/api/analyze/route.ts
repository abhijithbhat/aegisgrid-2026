...
export async function POST(request: Request): Promise<Response> {
  ...
  const sourceTextById = Object.fromEntries(
    body.sources.map((source) => [source.sourceId, source.text]),
  );
  const runAnalysis = () =>
    analyzeWithProvider({
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        send("reasoning", {
          stage: "Evidence envelope validated",
          detail: `${body.sources.length} source IDs checked`,
        });
        await pause();
        send("reasoning", {
          stage: "Deterministic boundary preserved",
          detail: `Risk ${body.deterministicRisk.score}/100 and route treated as read-only evidence`,
        });
        await pause();
        send("reasoning", {
          stage: "Semantic synthesis running",
          detail:
            "Gemini is interpreting cross-source meaning; deterministic outputs remain available",
        });
        const outcome = await runAnalysis();
        const payload = finish(outcome);
        if (outcome.status === "available") {
          send("reasoning", {
            stage: "Semantic synthesis validated",
            detail: `${outcome.recommendation.evidence.length} citations · ${outcome.recommendation.contradictions.length} contradictions`,
          });
          await pause();
          send("reasoning", {
            stage: "Supervisor brief ready",
            detail: outcome.recommendation.summary,
          });
        } else {
          send("reasoning", {
            stage: "Degraded boundary active",
            detail: "No unvalidated recommendation was released",
          });
        }
        await pause();
        send("result", payload);
