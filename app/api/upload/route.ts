...
const ALLOWED_ZONE_IDS = new Set([
  "GATE-W", "GATE-N", "GATE-E", "GATE-S", "W-CONC", "N-CONC", "E-CONC", "S-CONC",
  "SEAT-NW", "SEAT-NE", "SEAT-SW", "SEAT-SE", "FOOD", "MEDICAL", "CONTROL",
  "ACCESS-CORR", "SERVICE-TUNNEL", "TRANSIT", "EXIT-NW", "EXIT-NE", "EXIT-SW", "EXIT-SE",
]);
...
const validateRequestSchema = z.object({
  ...
  rows: z.array(z.record(z.string().max(200), scalarSchema)).min(1).max(1_000),
...
const jsonActionSchema = z.discriminatedUnion("action", [validateRequestSchema, mappingPreviewSchema]);

const aiMappingResponseSchema = z.object({
  mappings: z.array(mappingSchema.extend({ source: z.literal("ai") })).min(1).max(40),
}).strict();
...
async function aiMappingsForUpload(
  ...
  try {
    const sampleRows = parsed.rows.slice(0, 4).map((row) => Object.fromEntries(
      Object.entries(row).map(([key, value]) => [key, typeof value === "string" ? value.slice(0, 2_000) : value]),
    ));
    ...
    if (sourceColumns.length !== parsed.headers.length || new Set(sourceColumns).size !== sourceColumns.length || parsed.headers.some((header) => !sourceColumns.includes(header))) return null;
    const targets = validated.data.mappings.flatMap((item) => item.canonicalField ? [item.canonicalField] : []);
...
async function responseForParsedUpload(parsed: Awaited<ReturnType<typeof parseUpload>>, id: string): Promise<Response> {
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (contentType.includes("application/json")) {
    ...
    structuredLog({ requestId: id, operation: "validate-import", outcome: result.accepted ? "success" : "rejected", durationMs: Date.now() - started, code: result.accepted ? undefined : "VALIDATION_FAILED" });
    return jsonResponse({ ok: result.accepted, stage: result.accepted ? "ready-to-import" : "validation-failed", validation: result }, result.accepted ? 200 : 422, id);
  }

  if (!contentType.includes("multipart/form-data")) {
    return publicError(id, 415, "UNSUPPORTED_MEDIA_TYPE", "Use multipart form data for files or JSON for validation.");
  }
  ...
  if (!(value instanceof File)) return publicError(id, 400, "FILE_REQUIRED", "Choose a CSV, JSON, PDF, or text file.");

  try {
    const parsed = await parseUpload(value);
    structuredLog({ requestId: id, operation: "inspect-upload", outcome: "success", durationMs: Date.now() - started });
    return await responseForParsedUpload(parsed, id);
  } catch (error) {
    if (error instanceof ImportValidationError) {
      structuredLog({ requestId: id, operation: "inspect-upload", outcome: "rejected", durationMs: Date.now() - started, code: error.code });
      return publicError(id, error.status, error.code, error.message);
    }
    structuredLog({ requestId: id, operation: "inspect-upload", outcome: "error", durationMs: Date.now() - started, code: "UPLOAD_FAILED" });
