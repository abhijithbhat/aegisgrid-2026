...
const incidentTypes = ["medical", "fire", "crowd", "security", "infrastructure", "accessibility", "lost_person", "other"] as const;
const requestSchema = z.object({
  ...
  if (new Set(ids).size !== ids.length) context.addIssue({ code: "custom", path: ["reports"], message: "Report source IDs must be unique." });
});
...
const ZONE_ALIASES: Record<string, string> = {
  "GATE-W": "gate-west", "W-GATE": "gate-west",
  "GATE-N": "gate-north", "N-GATE": "gate-north",
  "GATE-E": "gate-east", "E-GATE": "gate-east",
  "GATE-S": "gate-south", "S-GATE": "gate-south",
  "W-CONC": "concourse-west", "WEST_CONC": "concourse-west",
  "N-CONC": "concourse-north", "NORTH_CONC": "concourse-north",
  "E-CONC": "concourse-east", "EAST_CONC": "concourse-east",
  "S-CONC": "concourse-south", "SOUTH_CONC": "concourse-south",
  "ACCESS-CORR": "accessible-corridor", ACCESS_02: "accessible-corridor",
};
...
async function compareCandidate(
  ...
  try {
    const result = await generateStructuredJson({
      ...
      untrustedData: {
        reportA: { id: reportA.id, sourceId: reportA.sourceId, zoneId: reportA.zoneId, timestamp: reportA.timestamp, language: reportA.language, text: reportA.rawText },
        reportB: { id: reportB.id, sourceId: reportB.sourceId, zoneId: reportB.zoneId, timestamp: reportB.timestamp, language: reportB.language, text: reportB.rawText },
      },
    ...
    const idsMatch = parsed.data.reportAId === reportA.id && parsed.data.reportBId === reportB.id
...
export async function POST(request: Request): Promise<Response> {
  ...
  const candidates = generateDuplicateCandidates(reports, ZONE_EDGES, DEFAULT_FUSION_CONFIG.candidate);
  const reportById = new Map(reports.map((report) => [report.id, report]));
  const aiAvailable = geminiCapability().available;
  const decisions = aiAvailable
    ? (await Promise.all(candidates.slice(0, 12).map((candidate) => {
  ...
  return jsonResponse({
    ...
    aiNotice: aiAvailable ? undefined : "AI analysis unavailable. Only exact deterministic matches can be fused.",
    threshold: DEFAULT_FUSION_CONFIG.semanticConfidenceThreshold,
    candidates: result.candidates,
    clusters: result.clusters.map((cluster) => ({ id: cluster.id, reportIds: cluster.reportIds, sourceIds: cluster.sourceIds })),
