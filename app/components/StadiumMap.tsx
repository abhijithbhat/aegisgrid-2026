...
export function StadiumMap({ zones, selectedZone, selectedIncident, onSelectZone, pulseZoneId }: StadiumMapProps) {
      ...
      <div className="stadium-canvas">
        <svg viewBox="0 0 760 500" className="stadium-svg" aria-label="Interactive operations plan of Unity Stadium 2026">
          ...
          <g className="stadium-shell">
            ...
            {pulseCoords && (
              <g className="map-pulse-rings" transform={`translate(${pulseCoords.x} ${pulseCoords.y})`}>
                <circle className="pulse-ring ring-1" />
                <circle className="pulse-ring ring-2" />
              </g>
            )}

            <g className={`incident-marker ${pulseZoneId ? "incident-marker-flash" : ""}`} transform={`translate(${coords.x} ${coords.y})`} filter="url(#incidentPulse)">
