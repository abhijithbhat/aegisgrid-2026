...
async function capture(page, path) {
  ...
  await page.screenshot({ path, fullPage: true, animations: "disabled", style: ".topbar{position:relative!important}.sidebar{position:absolute!important}.skip-link{display:none!important}" });
}
...
for (const viewport of [{ label: "desktop", width: 1440, height: 900 }, { label: "mobile", width: 390, height: 844 }]) {
  ...
  const screenshotPath = (name) => fileURLToPath(new URL(`${name}-${viewport.label}.png`, output));
