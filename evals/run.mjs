  ...
  return {
    ...
    body: {
      ...
      deterministicRisk: {
        ...
        severity: provisionalSeverity,
        explanation: "Provisional deterministic evaluation score.",
      },
      sources: incident.reports.map((report) => ({
        sourceId: report.sourceId,
      ...
      route: {
        primaryZoneIds: [incident.zoneId],
        alternateZoneIds: [],
        etaMinutes: 0,
        avoidedZoneIds: [],
        rationale: "Route is outside this semantic evaluation.",
      },
    },
  };
}
...
function judge(testCase, response, body) {
  ...
  if (testCase.operation === "schema-map") {
    ...
    const unsafe = mappings.find(
      (mapping) => mapping.sourceColumn === testCase.expected.mustNotMap,
    );
    return {
      passed: body.requiresExplicitApproval === true && (!unsafe || unsafe.canonicalField === null),
      mode: body.aiAvailable ? "hybrid" : "deterministic",
    };
  }
  if (testCase.operation === "fuse") {
    const merged = (body.clusters ?? []).some((cluster) => cluster.sourceIds.length > 1);
    return {
      passed: merged === testCase.expected.shouldMerge && body.sourceReportsPreserved === true,
  ...
  const passed =
    recommendation.incidentType === expected.incidentType &&
...
if (results.every((result) => result.mode !== "unreachable")) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({ generatedAt, origin, results }, null, 2)}\n`,
  );
  const rows = results
    .map(
      (result) =>
        `| ${result.id} | ${result.passed ? "Pass" : "Fail"} | ${result.status} | ${result.mode} |`,
    )
    .join("\n");
  ...
  const updated = readme.replace(
    /<!-- eval-results:start -->[\s\S]*?<!-- eval-results:end -->/,
    table,
  );
