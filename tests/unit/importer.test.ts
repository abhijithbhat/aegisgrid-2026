...
describe("Data Injection Lab importer", () => {
  ...
  it("rejects negative capacity and occupancy without capacity", () => {
    ...
    const result = normalizeMappedRows([
      { timestamp: "2026-07-10T14:00:00Z", zone_id: "GATE-W", occupancy: 10, capacity: -3, event_phase: "ingress" },
      { timestamp: "2026-07-10T14:00:00Z", zone_id: "GATE-W", occupancy: 10, capacity: null, event_phase: "ingress" },
    ], proposeMappings(headers), zones);
  ...
  it("holds unknown zones for manual mapping", () => {
    const headers = ["timestamp", "zone_id", "occupancy", "capacity", "event_phase"];
    const result = normalizeMappedRows([
      { timestamp: "2026-07-10T14:00:00Z", zone_id: "UNKNOWN", occupancy: 10, capacity: 50, event_phase: "ingress" },
    ], proposeMappings(headers), zones);
