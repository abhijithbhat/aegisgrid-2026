  ...
  recommendedActions: [
    {
      ...
      targetMinutes: 1,
      justification: "The cited report describes unresponsiveness.",
    ...
    const provider: AIProvider = {
      ...
      generateStructured: ({ signal }) =>
        new Promise((_, reject) => {
          signal.addEventListener("abort", () => reject(new AIProviderFailure("TIMEOUT", true)), {
            once: true,
          });
        }),
    };
    const pending = analyzeWithProvider({
      ...baseInput,
      provider,
      timeoutMs: 1_000,
      maxAttempts: 1,
